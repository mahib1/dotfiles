...
toggle_mics() {
	...
	while IFS= read -r device; do
		...
		args+=(--add item mic.device.$COUNTER popup."$NAME"
			--set mic.device.$COUNTER label="${device}" icon="􀊱"
			label.color="$COLOR"
			click_script="SwitchAudioSource -t input -s \"${device}\" && sketchybar --set /mic.device\.*/ label.color=$WHITE --set \$NAME label.color=$WHITE --set $NAME popup.drawing=off")
		COUNTER=$((COUNTER + 1))
...
if [ "$BUTTON" = "left" ]; then
	...
	if [[ "$MIC_NAME" != "$VALIDATED_MIC_NAME" || -z "$MIC_NAME" ]]; then
		...
		sketchybar -m --set mic label="" icon=􀊲
	else
		# Update SketchyBar with the microphone's name and volume
		if [[ $MIC_VOLUME -lt 100 ]]; then
			osascript -e 'set volume input volume 100'
			sketchybar -m --set mic label="$MIC_NAME 100" icon=􀊱 icon.color=$WHITE label.color=$WHITE
		elif [[ $MIC_VOLUME -gt 0 ]]; then
			osascript -e 'set volume input volume 0'
			sketchybar -m --set mic label="$MIC_NAME 0" icon=􀊲 icon.color=$WHITE label.color=$WHITE
		fi
	fi
